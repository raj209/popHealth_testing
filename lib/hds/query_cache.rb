...
=begin
    ...
    # completed
    def patients_cached?
      !QualityReport.where({measure_id: self.measure_id,sub_id:self.sub_id, effective_date: self.effective_date, test_id: self.test_id, "status.state" => "completed" }).first.nil?
    end
...
=end
    ...
    def patient_results
     ...
     CQM::IndividualResult.where(patient_cache_matcher)
    end
    ...
    def patient_cache_matcher
      measure_id = Measure.where(hqmf_id: self.measure_id, sub_id: self.sub_id).pluck(:_id).first.to_s
      match = {'measure_id' => measure_id,
               ...
               'qdmpatient.extendedData.manual_exclusion' => {'$in' => [nil, false]}              
              }
=begin
      if(filters)
